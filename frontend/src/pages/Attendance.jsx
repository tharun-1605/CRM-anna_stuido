...
import { UserCheck, Clock, Coffee, StopCircle, PlayCircle, LogOut, Download } from 'lucide-react';
...
export default function Attendance() {
      ...
      <div className="max-w-3xl mx-auto mt-8 animate-fade-in-up">
        <div className="app-card p-6 md:p-10 flex flex-col items-center text-center relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 w-full mb-10 relative z-10">
            <div className="bg-white/40 backdrop-blur border border-white/60 rounded-3xl p-8 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-500">
              <div className="flex items-center justify-center mb-4 text-teal-600">
                <PlayCircle className="w-6 h-6 mr-2 opacity-80" />
                <p className="text-sm text-gray-500 font-bold uppercase tracking-widest">Clock In</p>
              </div>
              <p className="text-4xl font-black text-gray-800 tracking-tighter">{formatTime(todayRecord?.clockIn)}</p>
            </div>
            <div className="bg-white/40 backdrop-blur border border-white/60 rounded-3xl p-8 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-500">
              <div className="flex items-center justify-center mb-4 text-red-500">
                <LogOut className="w-6 h-6 mr-2 opacity-80" />
                <p className="text-sm text-gray-500 font-bold uppercase tracking-widest">Clock Out</p>
              </div>
              <p className="text-4xl font-black text-gray-800 tracking-tighter">{formatTime(todayRecord?.clockOut)}</p>
            </div>
          </div>
            ...
            <div className="mt-10 w-full text-left relative z-10">
              ...
              <div className="space-y-3">
                {todayRecord.breaks.map((b, i) => (
                  <div key={i} className="flex justify-between items-center text-sm bg-white/50 backdrop-blur-md border border-white/60 shadow-sm p-4 rounded-2xl hover:scale-[1.01] transition-transform">
                    <div className="flex items-center">
                       <div className={`w-3 h-3 rounded-full mr-3 shadow-sm ${b.type === 'Lunch' ? 'bg-orange-500' : 'bg-yellow-500'}`}></div>
                       <span className="font-black text-gray-800">{b.type}</span>
                    </div>
                    <span className="font-bold text-gray-600 bg-white/60 px-3 py-1.5 rounded-lg border border-white/60">{formatTime(b.startTime)} - {formatTime(b.endTime)}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  const renderAdminView = () => {
    const handleExport = () => {
      const exportData = adminRecords.map(r => ({
        Date: new Date(r.date).toLocaleDateString(),
        User: r.user?.name || 'Unknown',
        Email: r.user?.email || '',
        ClockIn: formatTime(r.clockIn),
        ClockOut: formatTime(r.clockOut),
        ActiveHours: (r.totalHours || 0).toFixed(2),
        Breaks: r.breaks?.length || 0,
        Status: r.status
      }));
      ...
      <div className="flex flex-col h-full space-y-4 md:space-y-6 py-2 md:py-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-2 animate-fade-in-up gap-4">
          ...
          <div className="w-full sm:w-auto flex space-x-3">
            ...
            />
            <button onClick={handleExport} className="bg-white border-2 border-gray-200 text-gray-700 hover:bg-gray-50 px-5 py-2 rounded-xl text-sm font-bold transition-all flex items-center shadow-sm whitespace-nowrap">
              ...
              <table className="w-full text-sm text-left whitespace-nowrap min-w-max">
                <thead className="text-[11px] text-gray-500 bg-white/30 border-b border-white/40 uppercase tracking-widest font-black">
                  <tr>
                    ...
                    <th className="px-6 py-5 text-center">Breaks</th>
                    <th className="px-6 py-5 text-center">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {adminRecords.map((r) => (
                     <tr key={r._id} className="hover:bg-white/40 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-center font-black text-teal-600">{formatTime(r.clockIn)}</td>
                        <td className="px-6 py-4 text-center font-black text-red-500">{formatTime(r.clockOut)}</td>
                        <td className="px-6 py-4 text-center">
                           <span className="bg-white/60 backdrop-blur text-gray-700 px-3 py-1.5 rounded-lg font-black border border-white/60 shadow-sm">
                        ...
                        <td className="px-6 py-4 text-center">
                          <span className="bg-emerald-100/50 backdrop-blur text-emerald-700 text-[10px] uppercase tracking-wider px-3 py-1.5 rounded-full font-bold border border-emerald-200">{r.status}</span>
                        </td>
                     </tr>
                  ))}
